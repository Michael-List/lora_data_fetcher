...
class ApiClient:
    ...
    def send_data(self, db, ws_data):
        ...
        if ws_data.temperature:
            r = self.session.post(url=self.api_url, params={'db': db}, data='temperature,station=' + str(ws_data.station) + ' value=' + str(ws_data.temperature) + ' ' + str(int((ws_data.timestamp.timestamp() * 1e+9))))
            if r.status_code not in range(200, 300):
                logging.warning('Could not post temperature, status code was: {}'.format(r.status_code))
                successful = False

        if ws_data.humidity:
            r = self.session.post(url=self.api_url, params={'db': db}, data='humidity,station=' + str(ws_data.station) + ' value=' + str(ws_data.humidity) + ' ' + str(int((ws_data.timestamp.timestamp() * 1e+9))))
            if r.status_code not in range(200, 300):
                logging.warning('Could not post humidity, status code was: {}'.format(r.status_code))
                successful = False

        if ws_data.pressure:
            r = self.session.post(url=self.api_url, params={'db': db}, data='pressure,station=' + str(ws_data.station) + ' value=' + str(ws_data.pressure) + ' ' + str(int((ws_data.timestamp.timestamp() * 1e+9))))
            if r.status_code not in range(200, 300):
